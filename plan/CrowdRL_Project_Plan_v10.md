...
Project Plan v10 — Draft for Internal Discussion
...
*Supersedes v9 (and v8 before it). v10 is a **correctness pass, not a redesign**: every
claim in this document was re-verified against the code on 2026-08-03, and roughly twenty
that had drifted were corrected — a batched-inference path the adapter never had, the
JuPedSim callback signature, the path-deviation definition, an MARL-library dependency that
was never taken, a curriculum table, and several "open" items that had quietly closed. The
design sections (1–8) are otherwise unchanged. Two additions of substance: Section 3.6 now
carries the **two-serialisation-schemes rationale** (why `config_io` exists alongside
`config_resolved.yaml`), rehomed from a handover note so it is maintained rather than
frozen; and the 2026-08-03 progress-log entry records the documentation consolidation,
including the doc-tree precedence rule that v9 flagged as an open action item.*

*Reading order: this document is canonical for current state. `docs/agent_pipeline.md` and
`docs/environment_mechanics.md` describe how the code behaves in mechanism-level detail.
`plan/archive/` holds dated point-in-time records that are **not** maintained.*
...
*Implementation status (2026-08-03).* The generator implements five tiers, not six:
`GeometryTier` is `TIER_0, TIER_1, TIER_2, TIER_3A, TIER_3B`
(`crowdrl_env/geometry_generator.py:34-39`) — Tier 3 is split into 3A (single room with
obstacles) and 3B (multi-room layouts). **Tiers 4 and 5 above are design, not code.** They
remain the intended direction; nothing depends on them today.
...
| IV | M9: JuPedSim integration | Package learned policy as a JuPedSim locomotion module. Open-source release with documentation and example notebooks. | Months 14–18 — **substantially delivered early** (2026-07-30): adapter, self-describing artefact, e2e scenarios and example notebook 10 are done and validated. Remaining: a packaged trajectory-level cross-model benchmark runner (notebook 11 is a committed first cut with one macroscopic metric) and the public release. |
