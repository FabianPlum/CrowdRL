...
> **Status: FUTURE WORK — idea capture only. Do NOT implement yet.**
> Not in scope for the `agent_dynamics_refactor` PR (#4). Committed on its own
> branch (`plan/waypoint-lookahead-idea`, off `master`) on 2026-06-18 so the idea
> is preserved independently of that PR.
